...
endpoint protection／EDRがadapterを削除、隔離、置換、または起動拒否した場合はデモを停止します。保護機能を無効化したり未承認の除外を追加したりせず、組織で承認された実行policyまたは署名済み配布方法が整うまでraw qualificationを合格にしません。
...
header が counter と status の storage をあらかじめ所有するため、ライブ修復で ABI や class layout は変わりません。checked-in baseline は安全ですが意図的に不完全で、最初の post-warm-up overrun で Warning にします。`CycleWatchTests.cpp` は要求どおりの三連続動作を表現しますが、project はテスト executable を build するだけで自動実行しません。Make attempt 0 では conditional `#error` の実compiler evidenceを取得し、その後だけ隔離workspace内の該当行を固定`#pragma message`へ一回置換します。これは機能修正と分離した人工faultの修復訓練です。
