...
Assert-Equal ([regex]::Matches($sourceText, '#error\s+MSBUILD_DEMO_ADAPTER_INTENTIONAL_COMPILER_FAULT\s+"demo/CycleWatch/src/CycleWatch\.cpp"').Count) 1 'Fault diagnostic carries the stable quoted Allowed File token for wrapper attribution'
$faultRepairLine = '#pragma message("MSBUILD_DEMO_ADAPTER_INTENTIONAL_FAULT_REPAIRED demo/CycleWatch/src/CycleWatch.cpp")'
Assert-Equal ([regex]::Matches($sourceText, 'AFTER_EVIDENCE_REPLACE_THIS_EXACT_LINE_WITH:\s*' + [regex]::Escape($faultRepairLine)).Count) 1 'Fault diagnostic names the exact one-time evidence-based repair line'
